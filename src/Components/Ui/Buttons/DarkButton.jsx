...
export default function DarkButton({ className, label, clickEvent, isLoading, disabled = false }) {
    return (
        label ? (
            <button disabled={!!isLoading || disabled} onClick={() => clickEvent()} className={`px-4 py-3 disabled:opacity-75 rounded-md bg-secondary-light hover:bg-secondary text-white flex items-center justify-center  ${className}`}>
